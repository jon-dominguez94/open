...
def proctition(arr, &prc)
    passed = []
    failed = []
    arr.each {|el| prc.call(el) ? passed << el : failed << el}
    passed + failed
end
...
def chain_map(val, prcs)
    (0...prcs.length).each { |i| val = prcs[i].call(val)}
    val
end
...
def proc_suffix(sent, proc_hash)
    words = sent.split
    words.map! do |word|
        suffix = ""
        proc_hash.each do |k,v|
            suffix += v if k.call(word)
        end
        word + suffix
    end
    words.join(" ")
...
def proctition_platinum(arr, *prcs)
    ...
    prcs.each_with_index do |prc, i|
        procced[i+1] = arr.select {|el| prc.call(el)}
        arr -= procced[i+1]
    end
    procced
end
...
def procipher(sent, proc_hash)
    words = sent.split
    words.each_with_index do |word,i|
        passed = []
        proc_hash.each do |k,v|
            passed << v if k.call(word)
        end
        passed.each {|prc| word = prc.call(word)}
        words[i] = word
    end
    words.join(" ")
end
...
def picky_procipher(sent, proc_hash)
   sent.split.map{|word| run_procs(word, proc_hash)}.join(" ")
end

def run_procs(word, proc_hash)
    proc_hash.each do |k,v|
